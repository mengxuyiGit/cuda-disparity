/* v3: implemented the baseline version:
     cost calculation + aggregation in one direction (L->R).
     (the sub3_kernel.cu without any suffix in CUDA-disparity-v3 is the same as this file.
     running the basic command in that folder will produce v3 results.)

    improvement from v3:
        + aggregation from UP/DOWN/L/R 4 directions
        + uniqueness check
*/
#include <stdio.h>
#include <algorithm>
...
void launch_sub3(float* c,
    ...
    */
    ...
    sub3_kernel<<<grid, block>>>(c, a, b, d_disparity, d_L, d_L_min, h, w, d_range);
